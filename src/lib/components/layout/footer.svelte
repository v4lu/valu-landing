...
<footer class="relative mt-24 border-t border-white/10 bg-[rgba(3,0,20,0.08)] backdrop-blur-[12px]">
	...
	<div class="container mx-auto px-6 pb-6 pt-8">
		<div class="mb-8 flex flex-col items-start justify-between lg:flex-row lg:items-center">
			...
			<div class="grid grid-cols-2 gap-x-8 gap-y-2 sm:grid-cols-4 lg:mx-4">
				{#each footerLinks as link}
					<a
						href={link.href}
						class="text-sm text-muted-foreground transition-colors hover:text-white hover:underline hover:underline-offset-4"
					>
						{link.title}
					</a>
				{/each}
		...
		<p class="w-full text-center text-xs leading-5 text-muted-foreground">
			© {currentYear} Valu Media, Inc. All rights reserved.
		</p>
	</div>
</footer>
